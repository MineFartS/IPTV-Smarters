    ...
    InHeadNoscript { // from class: org.jsoup.parser.HtmlTreeBuilderState.5
        ...
        public boolean process(Token token, HtmlTreeBuilder htmlTreeBuilder) {
            ...
            if (!token.isDoctype()) {
                ...
                } else if (token.isEndTag() && token.asEndTag().name().equals("noscript")) {
                    htmlTreeBuilder.pop();
                    htmlTreeBuilder.transition(HtmlTreeBuilderState.InHead);
                } else {
        ...
        public boolean process(Token token, HtmlTreeBuilder htmlTreeBuilder) {
            ...
            if (HtmlTreeBuilderState.isWhitespace(token)) {
                ...
                if (token.isStartTag()) {
                    ...
                    } else if (StringUtil.in(strName, "base", "basefont", "bgsound", "link", "meta", "noframes", "script", "style", ChartFactory.TITLE)) {
                        htmlTreeBuilder.error(this);
                    ...
                    } else if (strName.equals("head")) {
                        htmlTreeBuilder.error(this);
                        return false;
                    }
                    htmlTreeBuilder.transition(htmlTreeBuilderState);
                } else if (token.isEndTag() && !StringUtil.in(token.asEndTag().name(), "body", "html")) {
                    htmlTreeBuilder.error(this);
                    return false;
                }
