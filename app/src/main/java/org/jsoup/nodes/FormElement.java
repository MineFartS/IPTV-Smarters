...
public class FormElement extends Element {
            ...
            if (elementFirst.tag().isFormSubmittable() && !elementFirst.hasAttr("disabled")) {
                ...
                if (strAttr.length() != 0) {
                    ...
                    if ("select".equals(elementFirst.tagName())) {
                        ...
                        if (!z && (elementFirst = elementFirst.select("option").first()) != null) {
    ...
    public Connection submit() {
        ...
        return Jsoup.connect(strAbsUrl).data(formData()).method(attr("method").toUpperCase().equals(ClientConstants.HTTP_REQUEST_TYPE_POST) ? Connection.Method.POST : Connection.Method.GET);
    }
}
