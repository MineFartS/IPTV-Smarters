...
public class AmazonS3URI {
    ...
    private static void appendDecoded(StringBuilder sb, String str, int i2) {
        if (i2 <= str.length() - 3) {
            char cCharAt = str.charAt(i2 + 1);
            sb.append((char) (fromHex(str.charAt(i2 + 2)) | (fromHex(cCharAt) << 4)));
            return;
        }
    ...
    private static String decode(String str) {
        ...
        for (int i2 = 0; i2 < str.length(); i2++) {
            if (str.charAt(i2) == '%') {
                return decode(str, i2);
            }
        }
        return str;
    }

    private static String decode(String str, int i2) {
