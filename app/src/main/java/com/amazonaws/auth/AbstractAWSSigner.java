...
public abstract class AbstractAWSSigner implements Signer {
    ...
    public static final String EMPTY_STRING_SHA256_HEX = BinaryUtils.toHex(doHash(BuildConfig.FLAVOR));
    ...
    public byte[] getBinaryRequestPayloadWithoutQueryParams(Request<?> request) {
        ...
        try {
            ...
            while (true) {
                int i2 = binaryRequestPayloadStreamWithoutQueryParams.read(bArr);
                if (i2 == -1) {
    ...
    public String getCanonicalizedQueryString(Request<?> request) {
        return HttpUtils.usePayloadForQueryParameters(request) ? BuildConfig.FLAVOR : getCanonicalizedQueryString(request.getParameters());
    }
    ...
    public Date getSignatureDate(long j2) {
        Date date = new Date();
