    ...
    public synchronized void resumeAllTransfersOnNetworkAvailability() {
        ...
        try {
            ...
            while (cursorQueryTransfersWithTypeAndStates.moveToNext()) {
                int i3 = cursorQueryTransfersWithTypeAndStates.getInt(cursorQueryTransfersWithTypeAndStates.getColumnIndexOrThrow(TransferTable.COLUMN_ID));
                if (this.updater.getTransfer(i3) == null) {
            ...
            LOGGER.debug(i2 + " transfers are loaded from database.");
        } catch (Throwable th) {
