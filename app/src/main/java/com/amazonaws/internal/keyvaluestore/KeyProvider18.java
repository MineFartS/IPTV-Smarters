...
public class KeyProvider18 implements KeyProvider {
    ...
    private byte[] rsaDecrypt(String str, byte[] bArr) {
        try {
            ...
            logger.error("Exception occurred while decrypting the encrypted AES key. ", e2);
            return null;
        }
    }
    ...
    public synchronized void deleteKey(String str) {
        try {
            ...
            logger.error("Error in deleting the encrypted AES key identified by AesGcmNoPadding18-encrypted-encryption-key from SharedPreferences.", e2);
        }
        try {
            KeyStore keyStore = KeyStore.getInstance(ANDROID_KEY_STORE_NAME);
            keyStore.load(null);
            ...
            logger.error("Error in deleting the RSA Key identified by the keyAlias: " + str + " from " + ANDROID_KEY_STORE_NAME, e3);
        }
    }
    ...
    public synchronized Key generateKey(String str) {
        ...
        try {
            try {
                ...
                throw new KeyNotGeneratedException("Error in generating the RSA Encryption key for the rsaKeyAlias: " + str + " in " + ANDROID_KEY_STORE_NAME, e3);
            }
        } catch (Throwable th) {
            throw th;
        }
