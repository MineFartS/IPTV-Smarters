...
public final class AWSMobileClientAuthProvider implements CognitoAuthProvider {
    ...
    private AWSMobileClient getMobileClient() throws StorageException {
        try {
            ...
            throw new StorageException("AWSS3StoragePlugin depends on AWSCognitoAuthPlugin but it is currently missing", e2, "Before configuring Amplify, be sure to add AWSCognitoAuthPlugin same as you added AWSS3StoragePlugin.");
        }
    }
    ...
    public String getIdentityId() throws StorageException {
        try {
            ...
            throw new StorageException("Failed to get user's identity ID", e2, "Please check that you are logged in and that Auth is setup to support identity pools.");
        }
    }
}
